...
module.exports.getSignatureQueryString = function(signingFn, resource, config, signCb) {
	var argErrors = sanityCheckArgs(signingFn, resource, config);
	if (argErrors) {
		if (!signCb) {
	...
	var query = {
		"Signature": sanitizeForUrl(signingFn(config.privateKey, policyString)),
...
module.exports.signUrl = function (signingFn, resource, config, signCb) {
	var signatureQueryString = exports.getSignatureQueryString(signingFn, resource, config);
	var resourceInfo = getResourceInfo(resource);
...
function sanityCheckArgs (signingFn, resource, config) {
	var errors = [];
	var validProtocols = ['http:', 'https:', 'rtmp:', 'rtmpt:', 'rtmpe:', 'rtmpte:'];

	if (!signingFn) {
		errors.push('Error: signing function parameter is required');
	}
